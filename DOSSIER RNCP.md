...
<span class="token keyword">switch</span><span class="token punctuation">(</span><span class="token variable">$Action</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
	...
	<span class="token comment">// Exemple de récupération de plusieurs flows en même temps</span>
	...
	<span class="token comment">// Exemple de récupération du profil d'un utilisateur</span>
    ...
    <span class="token keyword">if</span><span class="token punctuation">(</span><span class="token operator">!</span><span class="token variable">$Result</span><span class="token punctuation">)</span> <span class="token variable">$Result</span> <span class="token operator">=</span> <span class="token string">'ERROR GET FLOW'</span><span class="token punctuation">;</span>
    <span class="token keyword">else</span> <span class="token function">Normalize</span><span class="token punctuation">(</span><span class="token variable">$Result</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token keyword">break</span><span class="token punctuation">;</span>

	<span class="token comment">// Exemple de connexion en tant qu'utilisateur à l'aide d'un token utilisateur</span>
    <span class="token keyword">case</span> <span class="token string">'ConnexionFromApi'</span><span class="token punctuation">:</span>
  
    <span class="token variable">$User</span> <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">User</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token variable">$Action</span> <span class="token operator">=</span> <span class="token function">get_action_from_post</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token variable">$Data</span> <span class="token operator">=</span> <span class="token function">get_all_from_post</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>

    <span class="token keyword">if</span><span class="token punctuation">(</span><span class="token operator">!</span><span class="token variable">$Result</span> <span class="token operator">=</span> <span class="token variable">$User</span><span class="token operator">-</span><span class="token operator">&gt;</span><span class="token function">ConnexionFromApi</span><span class="token punctuation">(</span><span class="token function">get_from_post</span><span class="token punctuation">(</span><span class="token string">'Token'</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
        <span class="token keyword">if</span><span class="token punctuation">(</span><span class="token variable">$User</span><span class="token operator">-</span><span class="token operator">&gt;</span><span class="token function">Add</span><span class="token punctuation">(</span><span class="token variable">$Action</span><span class="token punctuation">,</span><span class="token variable">$Data</span><span class="token punctuation">)</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
        ...
        <span class="token punctuation">}</span>
        <span class="token keyword">else</span> <span class="token variable">$Result</span> <span class="token operator">=</span> <span class="token string">'ERROR CONNEXION'</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
    <span class="token keyword">if</span><span class="token punctuation">(</span><span class="token function">is_object</span><span class="token punctuation">(</span><span class="token variable">$Result</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
        <span class="token function">set_user_connected</span><span class="token punctuation">(</span><span class="token variable">$Result</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token variable">$Result</span><span class="token operator">-</span><span class="token operator">&gt;</span><span class="token property">LinkBuilder</span> <span class="token operator">=</span> <span class="token constant">LINKBUILDER</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
    <span class="token keyword">break</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>

<span class="token comment">// Renvoi des données au front</span>
<span class="token keyword">if</span><span class="token punctuation">(</span><span class="token function">isset</span><span class="token punctuation">(</span><span class="token variable">$Result</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
    ...
    <span class="token function">exit</span><span class="token punctuation">(</span><span class="token function">json_encode</span><span class="token punctuation">(</span><span class="token variable">$Result</span><span class="token punctuation">,</span> <span class="token constant">JSON_UNESCAPED_UNICODE</span> <span class="token operator">|</span> <span class="token constant">JSON_UNESCAPED_SLASHES</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>

</code></pre>
<p><em>Extrait du fonctionnement de l’API du Serveur index.php</em></p>
<p>L’extrait ci-dessus illustre le fonctionnement de l’api. En fonction d’une action spécifiée dans la requête entrante, un traitement spécifique a lieu. En fonction de cette dernière un appel à nos différentes classes Flow ou User est fait par la création d’un objet du type de ces classes servant à communiquer avec les base de données. En effet, ces objets contiennent différentes fonctions chargés d’insérer, modifier ou supprimer des données directement en base. En passant par ces objets liés à la base, on manipule directement nos données en code tout en conservant une architecture orientée objet comme le permet des librairies comme LinQ en .NET par exemple. Ce principe de fonctionnement assure une cohérence et une robustesse aux données comme au code et facilite les échanges entre back et base de données.</p>
<hr>
<h3 id="securité">Securité</h3>
<p>Un point clé derrière la conception de notre infrastructure est la mise en place de sécurité fortes et cohérentes pour proteger à la fois nos données et nos scripts d’attaques et de failles.<br>
Nous avons évoqué les configurations d’éléments au travers d’apache et l’installation de certificats SSL permettant l’utilisation du protocole HTTPS sensiblement important dans la protection des échanges d’informations entre client et serveur puisqu’il prévient la récupération d’informations lors d’attaque de type Man in the middle, où un appareil redirige le traffic d’un réseau à travers lui en sniffant par la même occasion toutes les données non cryptées.<br>
Néanmoins ces première protections ne sont qu’une partie de celles déployées sur le serveur. En effet, tout le traffic envoyé à notre serveur est redirigé vers un seul et même fichier, impossible pour l’exterieur d’accèder un autre fichier que notre script php faisant office de gardien de nos fichiers. C’est ce dernier et seulement lui qui accède aux différents fichiers présents sur le serveur et ce si et seulement si les paramètres fournis dans la requête envoyée sont corrects et authentifiés avec succès. Cette configuration est permise notamment par un fichier .htacess permettant la spécifications de paramètres relatifs à apache et notamment au système de redirections.<br>
Pour accéder aux fichiers tels que les audios ou le fichiers images qu’ils soient relatifs aux photos de profil ou aux images de fond des flows, une procédure particulière est à suivre. En effet le lien vers cette image doit être accompagné de multiples paramètres dont des hash de vérification, le timestamp de la requête, le nom de l’image, etc… qui seront vérifiés par le serveur et en fonction de leur authenticité, ce dernier refusera ou donnera accès au fichier en question le tout uniquement par l’url de ce dernier.</p>
<hr>
<h3 id="communication-client--serveur">Communication Client / Serveur</h3>
<p>Pour communiquer avec le serveur et interroger notre api, j’ai mis en place une classe dédiée aux appels serveurs appelée ServerManager. Cette dernière permet de gérer toutes les actions nécessitant un envoie de données à l’api et est en charge de la récupération des informations renvoyées par le serveur. Pour élaborer un tel système, j’ai utilisé le framework JQuery qui permet ainsi d’utiliser un système appelé AJAX. Ce système permet de requeter une url en javascript, de passer des données à cette requête et de gèrer le retour de celle-ci à l’aide d’une fonction de callback.</p>
<pre class=" language-javascript"><code class="prism  language-javascript"><span class="token comment">// Classe de gestion des appels serveurs</span>
<span class="token keyword">class</span> <span class="token class-name">ServerManagerClass</span> <span class="token punctuation">{</span>
    <span class="token function">constructor</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span><span class="token punctuation">}</span>
    
    <span class="token comment">// Ajout de FLOW</span>
    <span class="token function">AddFlow</span><span class="token punctuation">(</span>data<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">let</span> final_data <span class="token operator">=</span> <span class="token punctuation">{</span>
            Data<span class="token punctuation">:</span> data<span class="token punctuation">,</span>
            Action<span class="token punctuation">:</span> <span class="token string">"AddFlow"</span><span class="token punctuation">,</span>
            TokenId<span class="token punctuation">:</span> window<span class="token punctuation">.</span>localStorage<span class="token punctuation">.</span><span class="token function">getItem</span><span class="token punctuation">(</span><span class="token string">"user_token"</span><span class="token punctuation">)</span>
        <span class="token punctuation">}</span><span class="token punctuation">;</span>

        $<span class="token punctuation">.</span><span class="token function">ajax</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
            type<span class="token punctuation">:</span> <span class="token string">"POST"</span><span class="token punctuation">,</span>
            url<span class="token punctuation">:</span> ServerParams<span class="token punctuation">.</span>ServerURL <span class="token operator">+</span> ServerParams<span class="token punctuation">.</span>AddFlowURL<span class="token punctuation">,</span>
            data<span class="token punctuation">:</span> JSON<span class="token punctuation">.</span><span class="token function">stringify</span><span class="token punctuation">(</span>final_data<span class="token punctuation">)</span><span class="token punctuation">,</span>
            success<span class="token punctuation">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span>response<span class="token punctuation">)</span> <span class="token punctuation">{</span>
                TLCurrentIndex <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span>
                ServerManager<span class="token punctuation">.</span><span class="token function">GetTimeline</span><span class="token punctuation">(</span><span class="token number">0</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
                <span class="token function">CloseAfterRecord</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token punctuation">}</span><span class="token punctuation">,</span>
            error<span class="token punctuation">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span>response<span class="token punctuation">)</span> <span class="token punctuation">{</span>
                <span class="token function">CloseAfterRecord</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token punctuation">}</span>
        <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>

	<span class="token comment">// Récupération d'un flow par son objectID</span>
    <span class="token function">GetFlowById</span><span class="token punctuation">(</span>id<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">let</span> final_data <span class="token operator">=</span> <span class="token punctuation">{</span>
            Data<span class="token punctuation">:</span> <span class="token punctuation">{</span>
                IdFlow<span class="token punctuation">:</span> id
            <span class="token punctuation">}</span><span class="token punctuation">,</span>
            Action<span class="token punctuation">:</span> <span class="token string">"GetSingle"</span><span class="token punctuation">,</span>
            TokenId<span class="token punctuation">:</span> window<span class="token punctuation">.</span>localStorage<span class="token punctuation">.</span><span class="token function">getItem</span><span class="token punctuation">(</span><span class="token string">"user_token"</span><span class="token punctuation">)</span>
        <span class="token punctuation">}</span><span class="token punctuation">;</span>

        $<span class="token punctuation">.</span><span class="token function">ajax</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
            type<span class="token punctuation">:</span> <span class="token string">"POST"</span><span class="token punctuation">,</span>
            url<span class="token punctuation">:</span> ServerParams<span class="token punctuation">.</span>ServerURL <span class="token operator">+</span> ServerParams<span class="token punctuation">.</span>GetSingleFlowURL<span class="token punctuation">,</span>
            data<span class="token punctuation">:</span> JSON<span class="token punctuation">.</span><span class="token function">stringify</span><span class="token punctuation">(</span>final_data<span class="token punctuation">)</span><span class="token punctuation">,</span>
            success<span class="token punctuation">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span>response<span class="token punctuation">)</span> <span class="token punctuation">{</span>
                console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">"Flow sucessfully recovered from database :"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
                console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span>response<span class="token punctuation">)</span><span class="token punctuation">;</span>
                <span class="token function">PopFlow</span><span class="token punctuation">(</span>response<span class="token punctuation">.</span>Data<span class="token punctuation">,</span> response<span class="token punctuation">.</span>LinkBuilder<span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token punctuation">}</span><span class="token punctuation">,</span>
            error<span class="token punctuation">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span>response<span class="token punctuation">)</span> <span class="token punctuation">{</span>
                console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">"Flow recovering from database error : "</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
                console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span>response<span class="token punctuation">)</span><span class="token punctuation">;</span>
                <span class="token function">pullToRefreshEnd</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token punctuation">}</span>
        <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
    
    <span class="token comment">// Ajout d'un commentaire à un flow</span>
    <span class="token function">AddFlowComment</span><span class="token punctuation">(</span>data<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">let</span> final_data <span class="token operator">=</span> <span class="token punctuation">{</span>
            Data<span class="token punctuation">:</span> data<span class="token punctuation">,</span>
            Action<span class="token punctuation">:</span> <span class="token string">"AddFlowComment"</span><span class="token punctuation">,</span>
            TokenId<span class="token punctuation">:</span> window<span class="token punctuation">.</span>localStorage<span class="token punctuation">.</span><span class="token function">getItem</span><span class="token punctuation">(</span><span class="token string">"user_token"</span><span class="token punctuation">)</span>
        <span class="token punctuation">}</span><span class="token punctuation">;</span>

        $<span class="token punctuation">.</span><span class="token function">ajax</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
            type<span class="token punctuation">:</span> <span class="token string">"POST"</span><span class="token punctuation">,</span>
            url<span class="token punctuation">:</span> ServerParams<span class="token punctuation">.</span>ServerURL <span class="token operator">+</span> ServerParams<span class="token punctuation">.</span>AddFlowComment<span class="token punctuation">,</span>
            data<span class="token punctuation">:</span> JSON<span class="token punctuation">.</span><span class="token function">stringify</span><span class="token punctuation">(</span>final_data<span class="token punctuation">)</span><span class="token punctuation">,</span>
            success<span class="token punctuation">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span>response<span class="token punctuation">)</span> <span class="token punctuation">{</span>
                <span class="token keyword">var</span> obj <span class="token operator">=</span> final_data<span class="token punctuation">.</span>Data<span class="token punctuation">;</span>
                obj<span class="token punctuation">.</span>IdComment <span class="token operator">=</span> response<span class="token punctuation">.</span>IdComment<span class="token punctuation">;</span>
                <span class="token function">send_comment_to_server</span><span class="token punctuation">(</span>obj<span class="token punctuation">)</span><span class="token punctuation">;</span>

            <span class="token punctuation">}</span><span class="token punctuation">,</span>
            error<span class="token punctuation">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span>response<span class="token punctuation">)</span> <span class="token punctuation">{</span>
                console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">"comment adding from database error : "</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
                console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span>response<span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token punctuation">}</span>
        <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
	
	<span class="token comment">// Récupération des commentaires d'un flow</span>
    <span class="token function">GetFlowComment</span><span class="token punctuation">(</span>data<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">let</span> final_data <span class="token operator">=</span> <span class="token punctuation">{</span>
            Data<span class="token punctuation">:</span> data<span class="token punctuation">,</span>
            Action<span class="token punctuation">:</span> <span class="token string">"GetFlowComment"</span><span class="token punctuation">,</span>
            TokenId<span class="token punctuation">:</span> window<span class="token punctuation">.</span>localStorage<span class="token punctuation">.</span><span class="token function">getItem</span><span class="token punctuation">(</span><span class="token string">"user_token"</span><span class="token punctuation">)</span>
        <span class="token punctuation">}</span><span class="token punctuation">;</span>

        console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span>final_data<span class="token punctuation">)</span><span class="token punctuation">;</span>
        $<span class="token punctuation">.</span><span class="token function">ajax</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
            type<span class="token punctuation">:</span> <span class="token string">"POST"</span><span class="token punctuation">,</span>
            url<span class="token punctuation">:</span> ServerParams<span class="token punctuation">.</span>ServerURL <span class="token operator">+</span> ServerParams<span class="token punctuation">.</span>GetFlowComment<span class="token punctuation">,</span>
            data<span class="token punctuation">:</span> JSON<span class="token punctuation">.</span><span class="token function">stringify</span><span class="token punctuation">(</span>final_data<span class="token punctuation">)</span><span class="token punctuation">,</span>
            success<span class="token punctuation">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span>response<span class="token punctuation">)</span> <span class="token punctuation">{</span>

                <span class="token function">get_all_comment</span><span class="token punctuation">(</span>response<span class="token punctuation">)</span><span class="token punctuation">;</span>
                console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">"Comment sucessfully added to database :"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
                console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span>response<span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<p><em>Extrait du code de ServerManager</em></p>
<p>Dans cet extrait, on met en valeur le fonctionnement de la classe ServerManager au travers de deux exemples de fonctionnalités. La récupération et insertion de flows sur le serveur, et l’insertion et la récupération de commentaires de flows. Pour utiliser de telles fonctionnalités un simple appel à la classe ServerManager est nécessaire :</p>
<pre class=" language-javascript"><code class="prism  language-javascript">ServerManager<span class="token punctuation">.</span><span class="token function">AddFlow</span><span class="token punctuation">(</span>data<span class="token punctuation">)</span>
</code></pre>
<p>La fonction correspondante à la fonctionnalité demandée reçoit alors en paramètres les informations nécessaire à son exécutions. Certaines données supplémentaires sont ajoutées ensuite à cet ensemble de données afin de normer ces dernières d’une certaines manière afin d’être correctement traitées par le serveur.</p>
<h2 id="veille">Veille</h2>
<p>Tout au long du projet, il a été essentiel pour toute l’équipe et moi même de s’informer sur les différentes avancées autour de domaines tels que le marché des applications mobiles mais aussi de technologies comme javascript, php et autres frameworks populaire. Cette recherche permanent d’informations constitue une veille technologique permanent notamment autour de la sécurité des technologies employé dans ce projet. Personnellement, je suis activement l’actualité du monde numérique au travers d’agrégateur d’articles de multiples sources autour de sujet que sont la sécurité informatique et l’évolution des technologies existantes. J’ai pu, au cours de ces deux dernières années, suivre notamment l’évolution du marché des applications mobiles. En effet, deux grandes catégories d’applications s’affrontent, dans un premier temps les applications dites natives, c’est à dire développer dans le langage natif aux deux systèmes d’exploitation que sont iOS et Android avec Swift pour le premier Java pour le second. Réputée pour sa rapidité, cette méthode voit néanmoins ses limites se dessiner récemment avec l’arrivée d’application hybrides utilisant des technologies web comme le propose Cordova permettant l’utilisation de modules natifs comme le microphone ou la caméra de l’appareil tout en proposant un seul et même code pour les deux plateformes et un développement semblable au développement web très apprécié des développeurs modernes. Néanmoins ces applications hybrides étaient jusque là assez moquées par les grandes entreprises et le marché en lui même pour leur manque de fluidité et leur mauvaise intégration au sein des différents écosystèmes d’Apple et de Google. Cependant, au cours des derniers mois des acteurs majeurs tels que Netflix ont décidé de proposer leur application en utilisant une application dite hybride relançant l’intérêt pour cette technologie de plus en plus populaire. De plus, les frameworks tels que Cordova ont pu durant ces dernières années, acquérir une maturité aujourd’hui redoutable permettant un développement solide et un documentation à la communauté très active. C’est l’ensembles de ces événements qui nous ont amené à faire le choix d’uen application hybride face à l’engouement des applications natives, prenant part à ce vent de fraîcheur que veulent apporter ces applications hybrides.<br>
En parallèle de l’actualité mobile, c’est l’évolution du langage Javascript que j’ai suivi au long de ces dernières années et particulièrement de ces derniers mois. Jugé langage le plus populaire sur GitHub avec plusieurs millions de projets réalisés avec ce dernier, la souplesse de ce langage adapté au web comme au développement d’applications mobile en passant par la réalisation de jeux vidéos a permis son explosion au sein de la communauté des développeurs du monde entier. Ses outils sont multiples, Javascript est un langage interprété et non compilé comme peut l’être certains de ces concurrents comme l’est Java ou le C#. Cela permet une vitesse d’execution incomparable et un environnement de developpement des plus agréables avec une console proposée notamment dans chrome inégalable proposant l’execution et le modification du code en temps réel et un affichage particulièrement détaillé. Ces derniers mois ont vu l’explosion de framework basés sur le langage avec pour les deux plus connus : AngularJS et ReactJS. Ces deux framework cumulent à eux seuls la quasi totalité des offres front disponibles sur le marché tant leur popularité grimpe en flèche. Pour des raisons de simplicité et après avoir pu benchmarker Angular, React et Vue, nous avons opter pour du javascript pur dans le projet pour la flexibilité qu’offre un developpement sans framework et pour limiter la complexité du projet et son poids au strict minimum. Ce choix nous permet de garder une forme de liberté totale sur le code et sur la réalisation de fonctionnalité comme le routing, gérer par chacun des framework ci dessus. Ainsi nous avons le contrôle total sur ces fonctionnalité et nous pouvons adapter ces dernières à nos besoin pour la plupart du temps, très spécifiques.<br>
Coté back, deux technologies marquent l’actualité récentes du développement back-end, celle de la démocratisation des bases NoSQL, et l’utilisation d’un framework qui explose avec l’avènement du big data, ElasticSearch. Les bases NoSQL entrent en compétition directe avec les modèles classiques de bases relationnelles. Ici, pas de clé étrangère ni primaire et une absence quasi totale de liens entre les différentes tables, en tout cas, en apparence. En effet, les bases NoSQL utilisent des liens différents pour corréler des données entre elles. Ces dernières sont pensées pour l’utilisation de Big Data et permettent la mise en place de framework récents tels que Elastic Search. Elastic Search est un module destiné à s’implémenter sur une base de données NoSQL permettant l’exploration de bases de données au nombre pharaonique de données en tout genre. Ce framework a la particularité de montrer des performances inégalables pour une base de données classique relationnelle et permet des requêtes particulièrement complexes. Elasitc Search permet de rajouter une forme d’intelligence derrière la recherche de données et s’est imposé comme un prérequis vital à la pratique de big data aujourd’hui au coeur des avancées en intelligence artificielle. C’est pour cette raison que nous avons opté dans un premier temps pour une base de données NoSQL justement à l’aide du récent mais non mois célèbre MongoDB, leader dans les solutions NoSQL, dans l’optique d’y implémenter un module Elastic Search gérant nos informations futures d’utilisateurs.<br>
Pourtant, très vite nous avons constaté que ce choix impliquait une complexification du back drastique, rendant quasi impossible le travail sur ce dernier pour un autre membre de l’équipe que le developpeur back spécialisé en MongoDB et en ElasticSearch. Par ailleurs, toujours dans un soucis de veille technologique, j’ai pu assister à différentes conférences de professionnels au cours de l’année, dont une présenté par un des fondateurs d’une des quatre startups française les plus florissantes, Doctolib. L’intervenant a alors insister sur leur volonté au sein de Doctolib d’aller à contre courant de la trop facile hype pour les nouvelles technologies vers lesquelles de nombreuses entreprises se tournent par effet de mode. Or Doctolib a toujours privilégier la simplicité à la complexité sous prétexte de modernité dans un contexte de développement, c’est pourquoi ils sont aujourd’hui encore, fiers d’utiliser une base de données relationnelles reposant sur PostGre, un système de gestion de base de données relativement ancien mais toujours très puissant mais boudé par les jeunes entreprise en soif de nouveautés et de framework plus tendances. Pourtant, Doctolib affiche une efficacité de traitement de ses données remarquables et par ce choix, assure une simplicité de maintenance de son environnement de développement extrêmement solide. C’est pourquoi, après avoir peser le pour et le contre, nous avons décider de reprendre le back à zéro et de redevelopper entièrement notre architecture en php MySQL, technologie que nous maîtrisons bien mieux et dont l’efficacité à déjà fait ses preuves bien que la tendance soit au NoSQL et à l’utilisation d’outils comme MongoDB ou Elastic Search.<br>
Toutes ces recherches constituent ma veille technologique permanente inhérente à ma passion pour l’informatique et les nouvelles technologies. Mes sources d’informations sont aussi multiples que variées, elle vont d’applications comme news republic, google news et flipboard à la recherche directe sur des sites spécialisés. Youtube et le visionnage de conférences constituent également une source d’information précieuses et quotidienne dans la constitution de ma veille.</p>
</div>
</body>
